...
function TrophyCase_Update()
	...
	for i = 1, TROPHYCASE_ITEMS_TO_DISPLAY do
		...
		if ( TrophyCaseDB.items[itemIndex] ) then
			...
			getglobal("TrophyCaseButton" .. i .. "Character"):SetText(nil);
			getglobal("TrophyCaseButton" .. i .. "Realm"):SetText(nil);
		end
	end
...
function TrophyCase_OnLoad()
	...
	SLASH_TROPHYCASE3 = "/tro";
	SLASH_TROPHYCASE4 = "/tc";
...
function TrophyCase_ScanItems()
	...
	-- DONE: can we tell from the link if an item still exists? No.
	...
	for bagID = KEYRING_CONTAINER, NUM_BAG_SLOTS + NUM_BANKBAGSLOTS do
		...
		for slot = 1, GetContainerNumSlots(bagID) do
			...
			if ( itemLink ) then
				-- TODO: why does having ClearLines() here cause bank items not to get a tooltip?
				--TrophyCaseScanningTooltip:ClearLines();
...
function TrophyCase_CheckItem(itemLink)
	...
	if ( IsEquippableItem(itemLink) ~= 1 ) then
		return;
	end
	...
	TrophyCase_DebugPrint("Before Tooltip: " .. itemLink);
	
	-- When CheckItem is called, the TrophyCaseScanningTooltip will have been set up by ScanItems already
	-- Since TrophyCaseScanningTooltip is defined in the XML, we use it like a global
	-- Check the tooltip for soulbound, quest item, starts a quest, or conjured item
	-- TODO: check for localized "Duration:"
	if ( not IsValidTooltip() ) then
		return;
	end
	
	TrophyCase_DebugPrint("After Tooltip: " .. itemLink);
...
function IsValidTooltip()
	...
	TrophyCase_DebugPrint("NumLines = " .. TrophyCaseScanningTooltip:NumLines());

	for i = 1, TrophyCaseScanningTooltip:NumLines() do
		local fontString = getglobal("TrophyCaseScanningTooltipTextLeft" .. i);
		
		TrophyCase_DebugPrint("TrophyCaseScanningTooltipTextLeft" .. i .. ": " .. fontString:GetText());
		
		if ( fontString:GetText() == ITEM_SOULBOUND ) then
			soulbound = true;
		end
		
		if ( fontString:GetText() == ITEM_CONJURED or
			 fontString:GetText() == ITEM_BIND_QUEST or
			 fontString:GetText() == ITEM_STARTS_QUEST ) then
			reject = true;
		end
	end
	
	if ( not soulbound or reject ) then
		return false;
	else
		return true;
	end
